...
describe("opencv-wasm", function () {
  ...
  it("runs dilation example", function () {
    ...
    const expectedOutput = PNG.sync.read(
      fs.readFileSync("./examples/expected-output/dilation.png")
    );
  ...
  it("runs template matching example", function () {
    ...
    const expectedOutput = PNG.sync.read(
      fs.readFileSync("./examples/expected-output/template-matching.png")
    );
    const testOutput = PNG.sync.read(
      fs.readFileSync("./examples/test-output/template-matching.png")
    );
...
describe("cvTranslateError", function () {
  it("translates error correctly", async function () {
    ...
    assert.deepStrictEqual(
      errString,
      "Exception: OpenCV(4.7.0-dev) /code/modules/imgproc/src/contours.cpp:197: error: (-210:Unsupported format or combination of formats) [Start]FindContours supports only CV_8UC1 images when mode != CV_RETR_FLOODFILL otherwise supports CV_32SC1 images only in function 'cvStartFindContours_Impl'\n"
    );
  });
});
